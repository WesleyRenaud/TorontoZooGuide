...
import {
   ScheduledPillChecker,
} from '../../../../../scripts/itinerary/panel/components/scheduledPillChecker.js';
...
test('Test_PlanScheduledPillRenderGroupsByAnchor_TestKeepsRealGapsAfterShortGroups_ExpectOk', () => {
   ...
   assert.equal(groups[0]?.displayDurationMinutes, ScheduledPillChecker.getScheduledPillMinDisplayMinutes());
   assert.equal(groups[1]?.durationMinutes, 2);
   assert.equal(groups[1]?.displayDurationMinutes, ScheduledPillChecker.getScheduledPillMinDisplayMinutes());
});
...
test('Test_PlanScheduledPillRenderGroupsByAnchor_TestGivesEachFullLengthVisitItsOwnPill_ExpectOk', () => {
   ...
   assert.equal(ScheduledPillChecker.MAX_TIMELINE_PILL_COLUMNS, 2);
   assert.equal(ScheduledPillChecker.MAX_TIMELINE_PILL_INDIVIDUAL_COLUMNS, 2);
});
