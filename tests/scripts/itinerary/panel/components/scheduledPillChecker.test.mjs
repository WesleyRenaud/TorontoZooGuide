...
test('Test_GetScheduledItemEndMinutes_TestUsesParsedEndMinutesFromScheduleTimes_ExpectOk', () => {
   assert.equal(
      ScheduledPillChecker.getScheduledItemEndMinutes({
         startMinutes: 600,
         maximumDuration: 8,
         endMinutes: 615,
      }),
   ...
   assert.ok(Number.isNaN(ScheduledPillChecker.getScheduledItemEndMinutes({
      startMinutes: 600,
      maximumDuration: 8,
   })));
});
...
test('Test_GetScheduledItemTimeRange_TestReturnsStartAndEndMinutes_ExpectOk', () => {
   assert.deepEqual(
      ScheduledPillChecker.getScheduledItemTimeRange({
         startMinutes: 570,
         endMinutes: 600,
      }),
      {
...
test('Test_DoScheduledTimeRangesOverlap_TestUsesStrictBoundsForTouchingWindows_ExpectOk', () => {
   assert.equal(
      ScheduledPillChecker.doScheduledTimeRangesOverlap(
...
test('Test_ComputeFirstFreeHorizontalOffsetIndex_TestReusesOpenColumns_ExpectOk', () => {
   ...
   assert.equal(
      ScheduledPillChecker.computeFirstFreeHorizontalOffsetIndex(
         placedItems,
         makeScheduledItem('Cheetah', 575, 30)
      ),
      1
...
test('Test_ComputeFirstFreeHorizontalOffsetIndex_TestReturnsPastMaxColumnWhenAllColumnsAreBlocked_ExpectOk', () => {
   ...
   assert.equal(
      ScheduledPillChecker.computeFirstFreeHorizontalOffsetIndex(placedItems, candidate),
...
test('Test_FormatScheduledPillGroupLabel_TestFormatsAnimalViewingSpotLabels_ExpectOk', () => {
   assert.equal(
      ScheduledPillChecker.formatScheduledPillGroupLabel([
...
test('Test_FormatScheduledPillGroupLabel_TestMatchesMapStyleCountedLabels_ExpectOk', () => {
   assert.equal(
      ScheduledPillChecker.formatScheduledPillGroupLabel([{ label: 'African Lion' }]),
      'African Lion'
   );
   assert.equal(
      ScheduledPillChecker.formatScheduledPillGroupLabel([
