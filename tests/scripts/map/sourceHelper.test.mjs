...
test('Test_NormalizeTypedRows_TestValidRows_ExpectTypedRows', () => {
   assert.deepEqual(SourceHelper.normalizeTypedRows([
   ...
   assert.deepEqual(SourceHelper.normalizeTypedRows(null, 'animal'), []);
});

test('Test_SetSourceRows_TestArrayRows_ExpectStored', () => {
   const store = _createStore();

   assert.deepEqual(SourceHelper.setSourceRows(store, 'animal', [{ species: 'African Lion' }]), [
      { species: 'African Lion' },
   ]);
   assert.deepEqual(store.byType.animal, [{ species: 'African Lion' }]);
   assert.deepEqual(SourceHelper.setSourceRows(store, 'giftShop', 'Zootique'), []);
});

test('Test_CreateDynamicTypedSource_TestRepeatedFetch_ExpectRefetchEachTime', async () => {
   const store = _createStore();
   let calls = 0;
   const source = SourceHelper.createDynamicTypedSource(store, 'attraction', async () => {
      calls += 1;
...
test('Test_CreateStaticTypedSource_TestSuccessfulFetch_ExpectCached', async () => {
   const store = _createStore();
   let calls = 0;
   const source = SourceHelper.createStaticTypedSource(store, 'giftShop', async () => {
      calls += 1;
...
test('Test_CreateStaticTypedSource_TestInFlightFailure_ExpectDedupeThenReset', async () => {
   ...
   const source = SourceHelper.createStaticTypedSource(store, 'wildEncounter', async () => {
      calls += 1;
