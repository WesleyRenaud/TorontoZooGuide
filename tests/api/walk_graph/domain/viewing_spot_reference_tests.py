...
from api.walk_graph.domain.viewing_spot_name_key import ViewingSpotNameKey
...
def Test_WalkNodeIdForStop_TestTransportation_ExpectResolvedNode(
      monkeypatch: pytest.MonkeyPatch ) -> None:
   monkeypatch.setattr(
...
def Test_WalkNodeIdForStop_TestKnownAttraction_ExpectWalkNodeId(
   ...
   monkeypatch.setattr(
      ...
      'for_map_location',
      lambda kind, name: MapLocationWalkNode(
...
def Test_StopsInMasterRouteLoopOrder_TestNoMappedStops_ExpectOriginalOrder(
      stub_loop_schedule_unit_builder_dependencies: None ) -> None:
   stops = [ UNKNOWN_ANIMAL ]

   ordered = LoopScheduleUnitBuilder._stops_in_master_route_loop_order(
      LOOPS_BY_ID[ 'africa_savanna_canadian_domain' ],
      stops )

   assert ordered == stops
