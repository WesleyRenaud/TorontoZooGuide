...
from api.restaurants.scheduling.restaurant_schedule_conflict_resolver import RestaurantScheduleConflictResolver
...
RESTAURANT = 'Simba Spot'
START_DATE = '2026-06-01'
END_DATE = '2026-06-30'
MESSAGE = 'Summer hours.'

def _schedule() -> RestaurantOpeningSchedule:
...
def _conflict_record() -> RestaurantScheduleRecord:
...
def Test_SaveReplacingOverlaps_TestSchedule_ExpectProviderBackedSaver(
   ...
   schedule = _schedule()
   saver_calls: list[ tuple[ RestaurantOpeningSchedule, Any ] ] = []

   def save_replacing_overlaps(
         _conn: Types.Connection,
         item: RestaurantOpeningSchedule,
         *,
         fetch_conflicts: Any,
         delete_conflict: Any,
...
def Test_SaveTrimmingOverlaps_TestSchedule_ExpectProviderBackedSaver(
   ...
   schedule = _schedule()
   saver_calls: list[ RestaurantOpeningSchedule ] = []
...
def Test_TrimConflict_TestConflictAndSchedule_ExpectTrimmerDelegation(
   ...
   schedule = _schedule()
   trim_calls: list[ tuple[ RestaurantScheduleRecord, RestaurantOpeningSchedule ] ] = []

   def trim(
         _conn: Types.Connection,
         item: RestaurantScheduleRecord,
         schedule_item: RestaurantOpeningSchedule,
         *,
         delete_conflict: Any,
         update_dates: Any,
         insert_copy: Any,
   ...
   RestaurantScheduleConflictResolver.trim_conflict(
      STUB_REQUEST_CONNECTION,
      conflict,
      schedule )
