...
def test_travel_time_minutes_from_length_px_rounds_to_nearest_minute() -> None:
...
def test_entrance_to_grizzly_bear_travel_time_is_about_thirty_minutes(
   ...
   assert expected_path.length_px == shortest_path_distance(
      ...
      walk_graph[ 'entrance_node_id' ],
      GRIZZLY_WALK_NODE_ID )
   assert 30 <= first_leg.travel_time_minutes <= 32
   assert first_leg.travel_time_minutes == travel_time_minutes_from_length_px(
      expected_path.length_px )
