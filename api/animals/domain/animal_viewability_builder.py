...
class AnimalViewabilityBuilder():
   ...
   def get_active_limited_viewing_status(
      ...
      is_active = DateValues.is_date_in_range(
         target_date=target_date,
         start_date_value=schedule_start_date,
         end_date_value=schedule_end_date )
   ...
   def get_active_viewing_alert_status(
      ...
      is_active = DateValues.is_date_in_range(
         target_date=target_date,
         start_date_value=alert_start_date,
         end_date_value=alert_end_date )
   ...
   def build_viewable_animals_on_day(
      ...
      built_animals = [
         cls.build_viewable_animal_from_record(
            animal_record,
            target_date=target_date,
            temp=temp,
            sigma=sigma )
      ...
      visible_animals = IndoorOutdoorViewingVisibilityBuilder.apply( built_animals )
   ...
   def build_viewable_animal_from_record(
      ...
      is_off_display, off_display_message = cls.get_active_off_display_status(
         animal=animal,
         target_date=target_date )

      has_limited_viewing_schedule, limited_viewing_message = cls.get_active_limited_viewing_status(
         animal=animal,
         target_date=target_date )

      _, viewing_alert_message = cls.get_active_viewing_alert_status(
         animal=animal,
         target_date=target_date )
      viewing_alert_messages = ValueConversion.as_singleton_list(
         viewing_alert_message )

      exhibit_status, exhibit_closed_message = cls.get_active_exhibit_status(
         animal=animal,
         target_date=target_date )

      likelihood = cls._calculate_viewable_animal_likelihood(
         animal,
         temp=temp,
         sigma=sigma,
         is_off_display=is_off_display,
         exhibit_status=exhibit_status )
      display_message = cls._get_viewable_animal_display_message(
         animal,
   ...
   @classmethod
   def _calculate_viewable_animal_likelihood(
         cls,
         animal: AnimalViewabilityRecord,
         temp: float,
         sigma: int,
      ...
      return cls.calculate_animal_likelihood(
         temp=temp,
         sigma=sigma,
         enclosure_type=animal.enclosure_type,
   ...
   @classmethod
   def _get_viewable_animal_display_message(
         cls,
         animal: AnimalViewabilityRecord,
         likelihood: int,
         is_off_display: bool,
