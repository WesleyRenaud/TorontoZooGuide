...
class LoopWindowPacker():
   ...
   def pack(
      ...
      return cls._pack_loops_for_anchored_window(
         walk_graph,
         schedule_window,
   ...
   def _pack_loops_for_anchored_window(
         ...
         *,
         ...
         current_node_id: str,
         departure_side_cluster_id: str | None = None ) -> list[ PreparedLoopScheduleUnit ]:
      anchor_node_id = cls._anchor_walk_node_id( schedule_window )

      if anchor_node_id is None:
         return cls._pack_loops_for_open_window(
            walk_graph,
            schedule_window,
            prepared_units=prepared_units,
            window_start_seconds=window_start_seconds,
      ...
      for terminal_unit in prepared_units:
         for oriented_terminal_unit in cls._prepared_unit_orientations( terminal_unit ):
            sequence = cls._pack_loops_with_terminal_unit(
               walk_graph,
               prepared_units,
         ...
         score = cls._anchored_sequence_score(
            walk_graph,
            sequence,
            window_end_seconds=schedule_window.end_seconds,
            window_start_seconds=window_start_seconds,
   ...
   def pack_all_before_deadline(
      ...
      packed_units = cls._pack_loops_for_open_window(
         walk_graph,
   ...
   def _pack_loops_with_terminal_unit(
      ...
      prefix_units = cls._greedy_prefix_units_before_terminal(
         walk_graph,
         [
            unit
            for unit in prepared_units
            if not cls._prepared_units_share_loop( unit, terminal_unit )
         ],
         window_start_seconds=window_start_seconds,
   ...
   def _greedy_prefix_units_before_terminal(
      ...
      while remaining_units:
         ...
         prefer_side_cluster_loop_order = cls._should_prefer_side_cluster_loop_order(
            fitting_units,
            previous_side_cluster_id=previous_side_cluster_id )

         next_unit = min(
            fitting_units,
            key=lambda unit: cls._prefix_unit_sort_key(
               walk_graph,
               unit,
         ...
         next_unit = cls._prepared_unit_with_best_approach_orientation(
            walk_graph,
            from_node_id=walk_node_id,
   ...
   def _pack_loops_for_open_window(
      ...
      while remaining_units:
         ...
         next_unit = min(
            fitting_units,
            key=lambda unit: cls._open_window_unit_sort_key(
               walk_graph,
               unit,
               from_node_id=walk_node_id,
               previous_side_cluster_id=previous_side_cluster_id,
   ...
   def _anchored_sequence_score(
      ...
      event_travel_distance = cls._walk_distance_px(
         walk_graph,
   ...
   def _prefix_unit_sort_key(
      ...
      return cls._prepared_unit_sort_key(
         walk_graph,
         prepared_unit,
   ...
   def _open_window_unit_sort_key(
      ...
      distance_rank, cluster_rank, loop_id = cls._prepared_unit_sort_key(
         walk_graph,
         prepared_unit,
         from_node_id=from_node_id,
         adjacency=adjacency,
   ...
   def _prepared_unit_sort_key(
      ...
      travel_distance = cls._travel_distance_to_unit_entry(
         walk_graph,
         from_node_id=from_node_id,
   ...
   def _choose_side_cluster_packing_order(
      ...
      soft_pin_loop_ids = cls._soft_pin_loop_ids_in_units(
         schedule_window,
         prepared_units )
      ...
      if cls._soft_pins_fit_before_close(
            schedule_window,
            prepared_units=prepared_units,
   ...
   def _travel_distance_to_unit_entry(
      ...
      reverse_distance = cls._walk_distance_px(
         walk_graph,
         from_node_id,
         exit_walk_node_id,
         adjacency=adjacency )
   ...
   def remove_matching(
      ...
      for index, candidate in enumerate( prepared_units ):
         if cls._prepared_units_share_loop( candidate, prepared_unit ):
   ...
   def _prepared_unit_orientations(
      ...
      return [
         cls._prepared_unit_with_loop_schedule_unit(
            prepared_unit,
            loop_unit )
   ...
   def _prepared_unit_with_best_approach_orientation(
      ...
      if reverse_distance < forward_distance:
         return cls._prepared_unit_with_loop_schedule_unit(
            prepared_unit,
