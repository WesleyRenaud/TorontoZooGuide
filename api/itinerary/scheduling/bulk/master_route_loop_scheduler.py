...
from .loop_schedule_window_state import LoopScheduleWindowState
...
class MasterRouteLoopScheduler():
   @classmethod
   def schedule(
         cls,
         conn: Connection,
      ...
      if prepared_units is None:
         return cls._animals_from_loop_units( loop_units ), schedule_cursor_seconds
      ...
      while remaining_units and window_index < len( schedule_windows ):
         window_index = cls._window_index_after_cursor(
            schedule_windows,
         ...
         if not cls._window_has_available_time(
               schedule_window,
               cursor_seconds=window_state.cursor_seconds ):
         ...
         pinned_loop_ids = cls._pinned_loop_ids_in_window( schedule_window )
         remaining_soft_pins = cls._remaining_soft_pins_in_window(
            schedule_window,
            remaining_units )
         active_soft_pin_loop_ids = cls._active_soft_pin_loop_ids( remaining_soft_pins )
         ...
         wait_constrained_loop_ids = (
            ...
            | cls._inactive_soft_pin_loop_ids_opening_before_active(
               schedule_window,
               remaining_units,
               active_soft_pin_loop_ids=active_soft_pin_loop_ids,
               cursor_seconds=window_state.cursor_seconds ) )
         should_abort = not cls._process_schedule_window(
            conn,
         ...
         wait_until_seconds = cls._earliest_pinned_loop_wait_seconds(
            remaining_units,
            wait_constrained_loop_ids,
            pinned_earliest_start_cache=pinned_earliest_start_cache,
            cursor_seconds=window_state.cursor_seconds )
      ...
      return (
         cls._animals_from_prepared_units( remaining_units ),
   ...
   def _process_schedule_window(
      ...
      active_open_seconds = cls._earliest_active_soft_pin_open_seconds(
         schedule_window,
         active_soft_pin_loop_ids )
      wait_filler_pending = (
      ...
      if hard_pinned_loop_ids and not wait_filler_pending:
         packed_cursor_seconds, should_abort = cls._pack_non_pinned_loops_before_pinned_deadline(
            conn,
            remaining_units=remaining_units,
            schedule_window=schedule_window,
      ...
      if hard_pinned_loop_ids and not wait_filler_pending:
         window_state.cursor_seconds = cls._drain_ready_pinned_loop_units(
            conn,
            remaining_units,
            schedule_window,
            pinned_earliest_start_cache=pinned_earliest_start_cache,
      ...
      if cls._should_defer_free_packing_until_after_anchor(
            schedule_window,
      ...
      hard_pin_deadline_seconds = cls._earliest_hard_pin_deadline_seconds(
         remaining_units,
      ...
      if (
         ...
         window_state.cursor_seconds, window_state.current_node_id = (
            cls._drain_ready_soft_pin_loop_units(
               conn,
               remaining_units,
               schedule_window,
      ...
      pack_open_soft_pins_with_free_loops = cls._should_pack_open_soft_pins_with_free_loops(
         remaining_units=remaining_units,
         active_soft_pin_loop_ids=active_soft_pin_loop_ids,
         held_constrained_loop_ids=held_constrained_loop_ids,
      ...
      if remaining_hard_pin_loop_ids:
         packing_hold_loop_ids |= cls._later_same_cluster_loop_ids(
            remaining_units,
            remaining_hard_pin_loop_ids )
      elif schedule_window.opens_after_fixed_time_stop:
         packing_hold_loop_ids |= cls._side_cluster_successor_loop_ids(
            remaining_units,
      ...
      packing_window = cls._non_pinned_packing_window(
         schedule_window,
         remaining_units=remaining_units,
         pinned_loop_ids=hard_pinned_loop_ids,
         pinned_earliest_start_cache=pinned_earliest_start_cache,
      ...
      if wait_filler_pending and active_open_seconds is not None:
         ...
         wait_pack_end_seconds, wait_pack_planned_active_start_seconds = (
            cls._wait_filler_pack_end_seconds(
               schedule_window,
               remaining_units=remaining_units,
               active_soft_pin_loop_ids=active_soft_pin_loop_ids,
               hard_pinned_loop_ids=hard_pinned_loop_ids,
         ...
         packing_window = cls._packing_window_with_active_soft_pin_tail_reserve(
            packing_window,
            schedule_window=schedule_window,
            remaining_units=remaining_units,
            active_soft_pin_loop_ids=active_soft_pin_loop_ids,
      ...
      packed_units = LoopWindowPacker.pack(
         walk_graph,
         packing_window,
         prepared_units=cls._units_excluding_pinned_loops(
            remaining_units,
      ...
      if packed_units:
         ...
         window_state.cursor_seconds = cls._schedule_start_seconds_for_packed_units(
            packing_window,
         ...
         for prepared_unit in packed_units:
            ...
            try:
               unscheduled_animals = cls._schedule_prepared_loop_unit(
                  conn,
                  prepared_unit,
                  blockers=blockers,
            ...
            LoopWindowPacker.remove_matching( remaining_units, prepared_unit )
            window_state.cursor_seconds = (
               unit_start_seconds + prepared_unit.occupied_seconds )
      ...
      if wait_filler_pending:
         ...
         if cascade_end_seconds is not None:
            ...
            window_state.cursor_seconds, window_state.current_node_id = (
               cls._drain_cascaded_inactive_soft_pin_loop_units(
                  conn,
                  remaining_units,
                  schedule_window,
                  active_soft_pin_loop_ids=active_soft_pin_loop_ids,
         ...
         if cls._units_matching_loop_ids(
               remaining_units,
               active_soft_pin_loop_ids ):
            return True
   ...
   def _wait_filler_pack_end_seconds(
      ...
      """
      inactive_before_active_loop_ids = cls._inactive_soft_pin_loop_ids_before_active(
         schedule_window,
         remaining_units,
         active_soft_pin_loop_ids=active_soft_pin_loop_ids,
         active_open_seconds=active_open_seconds )
      inactive_reserve_seconds = sum(
         ...
         for prepared_unit in cls._units_matching_loop_ids(
            remaining_units,
   ...
   def _drain_cascaded_inactive_soft_pin_loop_units(
      ...
      for soft_pin in inactive_pins:
         matching_units = cls._units_matching_loop_ids(
            remaining_units,
            { soft_pin.loop_id } )
         ...
         if not cls._units_matching_loop_ids( remaining_units, { soft_pin.loop_id } ):
   ...
   def _packing_window_with_active_soft_pin_tail_reserve(
      ...
      active_units = cls._units_matching_loop_ids( remaining_units, soft_only_loop_ids )
      ...
      hard_pin_start_seconds = cls._earliest_pinned_loop_wait_seconds(
         remaining_units,
         hard_pinned_loop_ids,
         pinned_earliest_start_cache=pinned_earliest_start_cache,
   ...
   def _should_defer_free_packing_until_after_anchor(
      ...
      free_units = cls._units_excluding_pinned_loops(
         remaining_units,
         held_pinned_loop_ids )
      ...
      for later_window in later_schedule_windows:
         ...
         reserved_start_seconds = cls._earliest_pinned_loop_wait_seconds(
            remaining_units,
            {
               loop_pin.loop_id
   ...
   def _pack_non_pinned_loops_before_pinned_deadline(
      ...
      if schedule_window.opens_after_fixed_time_stop:
         held_loop_ids |= cls._side_cluster_successor_loop_ids(
            remaining_units,
            pinned_loop_ids )

      non_pinned_units = cls._units_excluding_pinned_loops(
         remaining_units,
         held_loop_ids )
      ...
      pinned_deadline_seconds = cls._earliest_pinned_loop_wait_seconds(
         remaining_units,
         pinned_loop_ids,
         pinned_earliest_start_cache=pinned_earliest_start_cache,
         cursor_seconds=window_state.cursor_seconds )
   ...
   def _drain_ready_pinned_loop_units(
      ...
      while True:
         ...
         for prepared_unit in cls._ready_pinned_loop_units(
               remaining_units,
               pinned_loop_ids,
               pinned_earliest_start_cache=pinned_earliest_start_cache,
               cursor_seconds=schedule_cursor_seconds ):
            ...
            if cls._keep_partial_pinned_loop_progress(
                  conn,
                  remaining_units,
                  prepared_unit,
   ...
   def _drain_ready_soft_pin_loop_units(
      ...
      if soft_only_loop_ids is None:
         soft_pin_loop_ids = cls._soft_pin_loop_ids_in_window( schedule_window )
      ...
      while True:
         ...
         for prepared_unit in cls._ready_pinned_loop_units(
            ...
            if cls._keep_partial_soft_pin_loop_progress(
                  conn,
                  remaining_units,
                  prepared_unit,
   ...
   def _keep_partial_pinned_loop_progress(
      ...
      replacement = cls._prepared_loop_unit_from_stops(
         conn,
         prepared_unit.unit,
         unscheduled_animals )
   ...
   def _later_same_cluster_loop_ids(
      ...
      for soft_pin_unit in cls._units_matching_loop_ids( prepared_units, soft_pin_loop_ids ):
   ...
   def _window_index_after_cursor(
         ...
         *,
         window_index: int,
         cursor_seconds: int ) -> int:
      while cls._cursor_has_passed_schedule_window(
            schedule_windows,
            window_index=window_index,
            cursor_seconds=cursor_seconds ):
         window_index += 1
