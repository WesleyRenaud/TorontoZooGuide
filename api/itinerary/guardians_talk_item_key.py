...
GUARDIANS_TALK_ITEM_KEY_SEPARATOR = '||'
...
class GuardiansTalkScheduleItemKey:
   ...
   def from_wire( cls, wire: str ) -> Self | None:
      parts = wire.split( GUARDIANS_TALK_ITEM_KEY_SEPARATOR, 2 )
      name = parts[ 0 ].strip()
   ...
   def __post_init__( self ) -> None:
      ...
      if not normalized_start_time:
         raise ValueError( 'Invalid guardians talk start time: ' + repr( self.start_time ) )
      ...
      if normalized_end_time is None:
         raise ValueError( 'Invalid guardians talk end time: ' + repr( self.end_time ) )
   ...
   def from_row( cls, row: Any ) -> Self | None:
      if hasattr( row, 'talk_name' ):
         name = row.talk_name
         start_time = row.start_time
         end_time = row.end_time
      else:
         source = row if isinstance( row, dict ) else {}
         name = source.get( 'talk_name' ) or source.get( 'name' ) or ''
   ...
   def __eq__( self, other: Any ) -> bool:
      if not isinstance( other, GuardiansTalkScheduleItemKey ):
   ...
   def to_wire( self ) -> str:
      ...
      return GUARDIANS_TALK_ITEM_KEY_SEPARATOR.join( parts )
