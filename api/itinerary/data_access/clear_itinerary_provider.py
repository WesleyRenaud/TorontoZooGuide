...
class ClearItineraryProvider():
   @classmethod
   def clear_itinerary_date( cls, cur: Types.Cursor ) -> None:
      cur.execute( 'DELETE FROM ItineraryDate;' )


   @classmethod
   def clear_itinerary_exhibits( cls, cur: Types.Cursor ) -> None:
   ...
   @classmethod
   def clear_itinerary_attractions( cls, cur: Types.Cursor ) -> None:
      cur.execute( 'DELETE FROM ItineraryAttraction;' )


   @classmethod
   def clear_itinerary_guardians_talks( cls, cur: Types.Cursor ) -> None:
      cur.execute( 'DELETE FROM ItineraryGuardiansTalk;' )


   @classmethod
   def clear_itinerary_wild_encounters( cls, cur: Types.Cursor ) -> None:
      cur.execute( 'DELETE FROM ItineraryWildEncounter;' )


   @classmethod
   def clear_itinerary_events( cls, cur: Types.Cursor ) -> None:
      cur.execute( 'DELETE FROM ItineraryEvent;' )


   @classmethod
   def clear_itinerary( cls, conn: Types.Connection ) -> bool:
      cur = conn.cursor()
