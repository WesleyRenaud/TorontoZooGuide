...
class TimedNameScheduleItemKey:
   NAME_FIELDS: ClassVar[ list[ str ] ] = [ 'name' ]
   LABEL: ClassVar[ str ] = 'schedule item'
   ...
   def __post_init__( self ) -> None:
      ...
      if not normalized_start_time:
         raise ValueError(
            'Invalid ' + self.LABEL + ' start time: ' + repr( self.start_time ) )
      ...
      if normalized_end_time is None:
         raise ValueError(
            'Invalid ' + self.LABEL + ' end time: ' + repr( self.end_time ) )
   ...
   def from_row( cls, row: Any ) -> Self | None:
      row_attribute = cls.NAME_FIELDS[ Position.FIRST ]

      if hasattr( row, row_attribute ):
         name = getattr( row, row_attribute )
   ...
   def to_wire( self ) -> str:
      ...
      return ScheduleItemKeySeparator.VALUE.join( parts )
