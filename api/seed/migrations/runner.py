...
def _table_columns( cursor: Cursor, table: str ) -> set[ str ]:
   return {
      row[ 1 ]
...
def _execute_migration_statement( cursor: Cursor, statement: str ) -> None:
...
def _record_migration( cursor: Cursor, migration_name: str ) -> None:
   cursor.execute(
...
def run_migrations_on_cursor(
   ...
   *,
   ...
   for migration_file in migration_files():
      ...
      for statement in _split_sql_statements( migration_file.read_text() ):
