...
from .walk_graph_spur import WalkGraphSpur
...
class WalkGraphSpurBuilder():
   @classmethod
   @lru_cache( maxsize=1 )
   def build( cls ) -> list[ WalkGraphSpur ]:
      return cls.build_for_graph( WalkGraphProvider.fetch() )


   @classmethod
   def build_for_graph( cls, graph: WalkGraph ) -> list[ WalkGraphSpur ]:
      return cls._from_graph( graph )


   @classmethod
   def index_for_viewing_node_ids(
         cls,
   ...
   @classmethod
   def _from_graph( cls, graph: WalkGraph ) -> list[ WalkGraphSpur ]:
      ...
      for bridge in cls._find_bridges( adjacency ):
         for attachment_node_id, spur_node_ids in cls._spur_sides_for_bridge(
               bridge,
               adjacency,
      ...
      return cls._merge_subset_spurs( spur_regions )
   ...
   def _spur_sides_for_bridge(
      ...
      for attachment_node_id, other_node_id in (
         ...
         main_component = cls._component_without_edge(
            attachment_node_id,
            bridge,
            adjacency )
         spur_component = cls._component_without_edge(
            other_node_id,
            bridge,
            adjacency )
