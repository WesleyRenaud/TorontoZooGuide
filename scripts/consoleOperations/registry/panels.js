...
export function createConsoleOperationsPanelsHtml() {
   return createPanelHtmlFns
      .map(createPanelHtml => createPanelHtml())
      .join('\n');
}

export function renderConsoleOperationsPanels(workspaceEl) {
   if (!workspaceEl) {
      return;
   }

   workspaceEl.innerHTML = createConsoleOperationsPanelsHtml();
}
