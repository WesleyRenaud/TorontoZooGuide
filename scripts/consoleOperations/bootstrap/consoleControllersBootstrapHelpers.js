...
export class ConsoleControllersBootstrapHelpers {
   ...
   static initAnimalSpeciesAutocompletes(animals) {
      ConsoleControllersBootstrapHelpers.ANIMAL_SPECIES_AUTOCOMPLETE_KEYS.forEach(key => {
   ...
   static wireControllerBindings({
      ...
      ConsoleControllersBootstrapHelpers.CONTROLLER_BINDINGS.forEach(({ createController, getRefs, getExtraOptions }) => {
         createController(
            ConsoleControllersBootstrapHelpers.createControllerOptions({
               refs: getRefs(refs),
               activatePanel,
               getExtraOptions,
               specialControllers,
