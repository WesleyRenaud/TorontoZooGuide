...
export class AnimalsRouter {
   static createAnimalsRouter({ listEl }) {
      ...
      async function showRegions() {
         await runNavigation(() => AnimalsClient.getRegions(), (regions) => {
      ...
      async function showExhibits(regionName) {
         await runNavigation(() => AnimalsClient.getExhibitsInRegion(regionName), (exhibits) => {
      ...
      async function showAnimals(regionName, exhibitName) {
         await runNavigation(() => AnimalsClient.getAnimalsInExhibit(exhibitName), (animals) => {
      ...
      async function showAnimalDetail(regionName, exhibitName, animalName) {
         await runNavigation(
            () => AnimalsClient.getAnimalInformation({ species: animalName, exhibit: exhibitName }),
            (animalInfo) => {
