import { ItineraryPathGeometryHelper } from './itineraryPathGeometryHelper.js';
import { WalkGraphPathCalculator } from './walkGraphPathCalculator.js';

export class ItineraryPathCalculator {
   ...
   static buildItineraryPathD(routePoints, segments = undefined) {
      ...
      const walkGraphSegments = (
         segments === undefined
            ? WalkGraphPathCalculator.getWalkGraphPathSegments()
            : segments
      );
      const walkGraphPathD = walkGraphSegments?.length
         ? WalkGraphPathCalculator.buildPathDFromWalkGraphSegments(walkGraphSegments, routePoints)
         : '';
      ...
      return ItineraryPathCalculator.buildSmoothedPathD(routePoints);
   }
   ...
   static inclusivePointSlicesForWalkRouteLegs(legs) {
      ...
      for (let legIndex = 0; legIndex < legs.length; legIndex += 1) {
         ...
         if (slices.length === 0) {
            fromPointSequence = 0;
         } else if (ItineraryPathGeometryHelper.legsShareJoinNode(legs[legIndex - 1], leg)) {
   ...
   static buildItineraryPathDFromWalkLegs(
      ...
      for (const [fromPointSequence, toPointSequence] of ItineraryPathCalculator.inclusivePointSlicesForWalkRouteLegs(legs)) {
         ...
         const legPathD = ItineraryPathCalculator.buildSmoothedPathD(routePoints);
