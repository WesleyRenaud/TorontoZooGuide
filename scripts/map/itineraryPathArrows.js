import { ItineraryPathArrowGeometry } from './itineraryPathArrowGeometry.js';
...
export class ItineraryPathArrows {
   static buildPathPolylines(pathD, stepPx = 8) {
      ...
      for (const segment of segments) {
         ...
         if (segment.tag === 'C') {
            ...
            ItineraryPathArrowGeometry.appendCubicBezierSamples(
               polyline,
               currentPoint,
               {
   ...
   static buildPathArrowPlacements(pathD, {
      ...
      const polylines = ItineraryPathArrowGeometry.mergeConnectedPolylines(
         ItineraryPathArrows.buildPathPolylines(pathD, curveSampleStepPx)
      );

      return polylines.flatMap((polyline) => ItineraryPathArrowGeometry.buildPathArrowPlacementsForPolyline(
         polyline,
         {
            intervalPx,
            skipEndPx,
