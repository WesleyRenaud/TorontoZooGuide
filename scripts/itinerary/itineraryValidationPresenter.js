...
export class ItineraryValidationPresenter {
   ...
   static aggregateAnimalsForVisibilityComparison(animals = []) {
      ...
      animals.forEach((animal) => {
         ...
         aggregatedBySpeciesExhibit.set(key, {
            ...existing,
            likelihood: ItineraryValidationPresenter.maxStoredLikelihood(existing.likelihood, animal.likelihood),
            old_likelihood: ItineraryValidationPresenter.maxStoredLikelihood(
   ...
   static isRemovedForValidation(item, animalMinLikelihood) {
      ...
      return (
         ItineraryValidationPresenter.hasStoredOldLikelihood(item)
         && after != null
   ...
   static buildRemovedAnimals(animals = [], animalMinLikelihood) {
      return animals
         .filter((animal) => ItineraryValidationPresenter.isRemovedForValidation(animal, animalMinLikelihood))
         .map(ItineraryValidationPresenter.withVisibilityFields);
   }
   ...
   static buildReducedVisibilityAnimals(
      ...
      return animals
         ...
         .filter((animal) => {
            ...
            return (
               ItineraryValidationPresenter.hasMeaningfulVisibilityChange(animal, visibilityChangeThreshold)
               && after < before
            );
         })
         .map(ItineraryValidationPresenter.withVisibilityFields);
   ...
   static buildRemovedAttractions(attractions = [], animalMinLikelihood) {
      return attractions
         .filter((attraction) => ItineraryValidationPresenter.isRemovedForValidation(attraction, animalMinLikelihood))
         .map(ItineraryValidationPresenter.withVisibilityFields);
   }
