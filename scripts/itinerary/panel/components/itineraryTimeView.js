...
import { ItineraryTimeInputHelper } from './itineraryTimeInputHelper.js';
import { Strings } from '../../../strings.js';
import { ValidationBubbleFragment } from '../../../validationBubbleFragment.js';

export class ItineraryTimeView {
   static makeItineraryTimeInput({
      ...
      const field = ItineraryPanelHelper.el('label', 'itinerary-day-time-control');
      const labelText = ItineraryPanelHelper.el('span', 'itinerary-day-time-control-label', label);
      const form = ItineraryPanelHelper.el('form', 'itinerary-day-time-form');
      const inputWrap = ItineraryPanelHelper.el('div', 'itinerary-day-time-input-wrap');
      const input = ItineraryPanelHelper.el('input', 'itinerary-day-time-input');
      const validationBubble = ValidationBubbleFragment.createValidationBubbleController({
         anchorEl: inputWrap,
      });
      ...
      async function saveSelectedTime(_selectedDates, dateStr, instance) {
         ...
         const nextValue = ItineraryTimeInputHelper.readPickerTimeValue(instance, dateStr, input)
            || latestPickerValue;
      ...
      syncClearButtonState();
      flatpickrInstance = ConsoleDateFactory.initTimePicker(input, {
         allowInput: false,
         onChange(_selectedDates, dateStr, instance) {
            latestPickerValue = ItineraryTimeInputHelper.readPickerTimeValue(instance, dateStr, input);
         },
         onOpen: () => {
            validationBubble.dismiss();
