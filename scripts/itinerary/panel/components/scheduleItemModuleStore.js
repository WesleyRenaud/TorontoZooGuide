...
export class ScheduleItemModuleStore {
   static canScheduleModuleSelection({
      selection = '',
      selectedRow = null,
      eventTypes = [],
   } = {}) {
      const effectiveSelection = ScheduleItemSearcher.resolveEffectiveScheduleItemSelection(selection, selectedRow);
   ...
   static filterVisibleScheduleModuleRows({
      ...
      return ScheduleItemSearcher.filterScheduleItemRowsForScheduleModule(rows, itinerary, {
   ...
   static shouldClearSelectedScheduleRow({
      ...
      return !visibleRows.some((row) => ScheduleItemSearcher.getScheduleItemRowId(row) === selectedRowId);
   }
