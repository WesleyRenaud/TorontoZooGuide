...
export class DayPlannerTimelineRenderer {
   static buildItineraryTimeMarkers(itinerary = {}, strings = {}) {
      ...
      return [
         {
            startMinutes: DayPlannerScheduleController.parseClockTimeMinutes(itinerary.arrivalTime),
            label: strings.arrivalLabel,
            kind: visitBoundaryEventTypes.arrival,
         },
         {
            startMinutes: DayPlannerScheduleController.parseClockTimeMinutes(itinerary.departureTime),
   ...
   static buildMarkersByAnchorSlot(
      ...
      return itineraryTimeMarkers.reduce((markersMap, marker) => {
         const anchorSlot = DayPlannerTimelineRenderer.findTimelineAnchorSlot(marker.startMinutes, sortedSlotStarts);
