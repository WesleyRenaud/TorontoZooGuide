...
export class DraftStorage {
   ...
   static ITINERARY_STORAGE_KEYS = [
      ...DraftStorage.ITINERARY_DRAFT_KEYS,
      ...DraftStorage.ITINERARY_SELECTION_KEYS,
   ];
   ...
   static loadArray(key) {
      const parsed = DraftStorage.safeParseJSON(localStorage.getItem(key), []);
   ...
   static isStoredItineraryStale() {
      const storedDate = DraftStorage.getStoredItineraryDate();

      if (!storedDate) return false;

      const normalizedStoredDate = DraftStorage.normalizeDateToLocalMidnight(storedDate);
   ...
   static clearItineraryDraftStorage({ includeSelections = true } = {}) {
      const keys = includeSelections
         ? DraftStorage.ITINERARY_STORAGE_KEYS
         : DraftStorage.ITINERARY_DRAFT_KEYS;
   ...
   static clearItinerarySelectionStorage() {
      DraftStorage.ITINERARY_SELECTION_KEYS.forEach((key) => {
         localStorage.removeItem(key);
      });
   }
   ...
   static removeAnimalFromItineraryAnimalDraft(itemType, key) {
      ...
      const remainingAnimals = DraftStorage.loadArray(StorageKeys.ANIMALS_KEY)
